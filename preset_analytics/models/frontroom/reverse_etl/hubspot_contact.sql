WITH base AS (
    SELECT
        hs_contact_id,
        MIN(user_id) AS preset_user_id,
        MAX(SPLIT(email_domain, ".")[OFFSET(0)]) AS preset_company,
        MAX(is_preset) AS is_preset,
        MAX(active) AS active,
        UNIX_MILLIS(MAX(CAST(DATE(last_login) AS TIMESTAMP))) AS last_login,
        SUM(login_count) AS login_count,
        SUM(fail_login_count) AS fail_login_count,
        MIN(created_dttm) AS created_dttm,
        MAX(hs_company_id) AS hs_company_id,
        SUM(dashboards_created_count) AS dashboards_created_count,
        SUM(non_example_dashboards_created_count) AS non_example_dashboards_created_count,
        SUM(dashboards_ltd_views) AS dashboards_ltd_views,
        SUM(dashboards_n28d_views) AS dashboards_n28d_views,
        SUM(dashboards_ltd_users) AS dashboards_ltd_users,
        SUM(dashboards_n28d_users) AS dashboards_n28d_users,
        MAX(has_dashboard_viewed_by_others) AS has_dashboard_viewed_by_others,
        SUM(charts_created_count) AS charts_created_count,
        SUM(non_example_charts_created_count) AS non_example_charts_created_count,
        ...
        SUM(invites_pending) AS invites_pending,
        SUM(invites_accepted) AS invites_accepted,
        MAX(l7) AS l7,
        MAX(l28) AS l28,
        SUM(ltd_visits) AS ltd_visits,
        TO_JSON_STRING(ARRAY_CONCAT_AGG(team_id_array)) AS team_ids,
        TO_JSON_STRING(ARRAY_CONCAT_AGG(team_role_array)) AS team_roles,
        MAX(highest_billing_status) AS highest_billing_status,
        MAX(is_team_creator) AS is_team_creator,
        MAX(has_superset_experience) AS has_superset_experience,
        MAX(department) AS department,
        MAX(role) AS role,
        CASE MAX(CASE company_size
        ...
        END AS company_size,
        MAX(highest_workspace_role) AS highest_workspace_role,
    FROM
      {{ ref('manager_user') }}
    WHERE
...
FROM
  base
LEFT JOIN {{ ref('hs_contact_events_agg_denorm') }} AS events
  ON base.hs_contact_id = events.hs_contact_id
